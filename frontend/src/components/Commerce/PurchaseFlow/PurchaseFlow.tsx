...
export function PurchaseFlow({artworks,onClose}:{artworks:Artwork[];onClose:()=>void}) {
  ...
  const formattedTotal=total!==null&&currency?formatPrice(total,currency):'Price on request';
  const reserveDays=artworks.map(artwork=>artwork.commerce?.reservationDays).filter((days):days is number=>Boolean(days)).sort((a,b)=>a-b)[0];
  const delivery=artworks.map(artwork=>artwork.commerce?.shipping?.estimatedBusinessDays).find(Boolean);
  const isReservation=Boolean(reserveDays);
  const allInsured=artworks.length>0&&artworks.every(artwork=>artwork.commerce?.shipping?.insured);
  const allWorldwide=artworks.length>0&&artworks.every(artwork=>artwork.commerce?.shipping?.worldwide);
  ...
  useLayoutEffect(()=>{
    ...
    const updateSlots=()=>{
      ...
      const artworkSize=parseFloat(styles.getPropertyValue('--purchase-artwork-size'))||82;
      const gap=parseFloat(styles.columnGap)||10;
      setArtworkSlots(Math.max(1,Math.floor((row.clientWidth+gap)/(artworkSize+gap))));
    };
    updateSlots();
  ...
  const visibleArtworks=artworks.slice(0,hasArtworkOverflow?Math.max(0,artworkSlots-1):artworkSlots);
  ...
  return <div className="purchase-flow-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
    <div className={`purchase-flow purchase-flow-${step}`} role="dialog" aria-modal="true" aria-label="Purchase flow" ref={dialogRef} tabIndex={-1}>
      ...
      {step==='summary'&&<>
        <div ref={artworksRef} className="purchase-flow-artworks" aria-label={`${artworks.length} ${artworks.length===1?'artwork':'artworks'} in your order`}>
          {visibleArtworks.map(artwork=><img key={artwork.id} src={artwork.imageUrl} alt={`${artwork.title} by ${artwork.artist}`} className="purchase-flow-artwork"/>)}
          {hasArtworkOverflow&&<span className="purchase-flow-artwork-overflow" aria-label={`${hiddenArtworkCount} more ${hiddenArtworkCount===1?'artwork':'artworks'}`}>•••</span>}
        </div>
        <p className="purchase-flow-artworks-artist">by Shir Zabolotny</p>
        <div className="purchase-flow-total"><span>Total</span><strong>{formattedTotal}</strong></div>
        <div className="purchase-flow-benefits">
          ...
          {allInsured&&<div><span><ParcelIcon/></span><p><strong>{allWorldwide?'Worldwide':'Careful'} shipping</strong>Carefully packaged and fully insured.</p></div>}
        </div>
      ...
      {step==='checkout'&&<form onSubmit={submit}>
        ...
        <button type="submit" className="purchase-flow-primary"><span aria-hidden="true">▣</span> {isReservation?`Reserve ${formattedTotal}`:`Pay ${formattedTotal}`}</button><p className="purchase-flow-terms">By placing your order, you agree to our <u>terms</u>.</p>
      </form>}
      {step==='confirmation'&&<>
        <div className="purchase-flow-success" aria-hidden="true">✓</div><h2 id="purchase-flow-title">Your {artworks.length===1?'piece is':'pieces are'}<br/>{isReservation?'reserved!':'yours!'}</h2><p className="purchase-flow-confirmation-copy">Thank you! We’ve received your order.<br/>You will receive a confirmation email shortly.</p>
      ...
      <div className="purchase-flow-artist-note"><img src={artistPortrait} alt="Shir Zabolotny"/><p><strong>Thank you for supporting original art.</strong>Every piece is created with passion and intention.</p><em>Shir Z.</em></div>
    </div>
  </div>;
}
