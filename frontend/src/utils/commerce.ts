...
export function getEffectivePrice(artwork: Artwork): number | null {
  ...
  return typeof artwork.salePrice === "number" &&
    artwork.salePrice >= 0 &&
...
export function getAcquisitionState(artwork: Artwork): AcquisitionState {
  if (
    getEffectivePrice(artwork) === null ||
    !isSafePurchaseUrl(artwork.purchaseUrl)
  )
    return "hidden";
...
export function getOfferDetails(artwork: Artwork) {
  ...
  return {
    ...
    label: artwork.commerce?.offerLabel ?? "Studio offer",
  };
}
