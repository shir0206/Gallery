...
export function CartPage({ artworks, onGallery, onShipping, onReturns, onContact, onRemove, onCheckout }: CartPageProps) {
  ...
  return <div className="cart-page">
    ...
    <footer className="cart-footer"><span className="cart-footer-mark">SZ</span><nav aria-label="Footer navigation"><button type="button" onClick={onGallery}>Artworks</button><button type="button" onClick={onShipping}>Shipping</button><button type="button" onClick={onReturns}>Returns</button><button type="button" onClick={onContact}>Contact</button></nav><span>© {new Date().getFullYear()} Shir Zabolotny</span></footer>
    <div className="cart-mobile-checkout"><div><span>Collection total</span><strong>{totalLabel}</strong></div><button type="button" onClick={onCheckout}>Checkout <span aria-hidden="true">→</span></button></div>
  </div>;
}
