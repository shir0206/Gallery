...
interface ArtworkPageProps { artwork: Artwork; onBack?: () => void; onScrollBack?: () => void; onPrevious?: () => void; onNext?: () => void; onAddToCart: (artworkId: string) => void; isCovered?: boolean; transitionPhase?: TransitionPhase; usesSharedArtwork?: boolean; browseDirection?: 'previous' | 'next' | null; isStaticPreview?: boolean; isDirectEntry?: boolean; }

export function ArtworkPage({ artwork, onBack, onScrollBack, onPrevious, onNext, onAddToCart, isCovered=false, transitionPhase='ready', usesSharedArtwork=false, browseDirection=null, isStaticPreview=false, isDirectEntry=false }: ArtworkPageProps) {
  ...
  const scrollToStage=useCallback((stage:'00'|'01'|'02')=>{
    ...
    if(!scroller)return;
    const overview=scroller.querySelector<HTMLElement>('.artwork-presentation-track');
    const details=scroller.querySelector<HTMLElement>('.details-section');
    if(!overview||!details)return;
    const top=stage==='00'
      ? overview.offsetTop
      : stage==='01'
        ? overview.offsetTop+Math.max(0,overview.offsetHeight-scroller.clientHeight)
        : details.offsetTop;
  ...
  return <div className="artwork-page" data-transition-phase={transitionPhase} data-intro-locked={isIntroLocked||undefined} data-static-preview={isStaticPreview||undefined} data-direct-entry={isDirectEntry||undefined} data-active-section="00" aria-hidden={isCovered||undefined}><div className="artwork-page-scroller" ref={scrollerRef}><main className="artwork-page-content" key={artwork.id} data-browse-direction={browseDirection||undefined}><ArtworkOverview artwork={artwork} transitionPhase={transitionPhase} usesSharedArtwork={usesSharedArtwork} browseDirection={browseDirection} onIntroReady={handleIntroReady} isStaticPreview={isStaticPreview}/><DetailsSection artwork={artwork} onAddToCart={onAddToCart}/></main></div><nav className="artwork-section-progress" aria-label="Artwork sections"><button type="button" onClick={()=>scrollToStage('00')} aria-label="Artwork preview">00</button><i className="artwork-progress-animation" aria-hidden="true"/><button type="button" onClick={()=>scrollToStage('01')} aria-label="Completed artwork presentation">01</button><i className="artwork-progress-sections" aria-hidden="true"/><button type="button" onClick={()=>scrollToStage('02')} aria-label="Artwork details">02</button></nav></div>;
}
