...
function orientationFromImage(artwork: Artwork): Artwork["orientation"] {
  ...
  const image = Array.from(document.images).find(
    (candidate) =>
      (candidate.currentSrc === artwork.imageUrl ||
...
function ArtworkFacts({ artwork }: { artwork: Artwork }) {
  ...
  return (
    <div className="overview-facts">
      <p className="overview-technical">{artwork.description.materials}</p>
...
export function ArtworkOverview({
  artwork,
  ...
  return (
    <section
      ref={trackRef}
      className="artwork-presentation-track"
      data-orientation={layoutOrientation}
      data-shape={layoutShape}
      data-transition-phase={transitionPhase}
      data-shared-artwork={usesSharedArtwork || undefined}
    ...
    >
      <div className="artwork-overview">
        ...
        <div className="overview-intro-meta" aria-hidden="true">
          <span>{artwork.artist}</span>
        </div>
        <div className="overview-cinematic-title" aria-hidden="true">
          {titleWords.map((word, index) => (
            <span
              className="overview-title-mask"
              key={`${word}-${index}`}
              style={{ "--word-index": index } as CSSProperties}
            >
              <span className="overview-title-word">{word}</span>
            </span>
          ))}
        </div>
        <p className="overview-scroll-hint">
        ...
        <figure
          ...
          style={
            {
              "--artwork-ratio": `${artwork.dimensions.width} / ${artwork.dimensions.height}`,
            } as CSSProperties
          }
        >
          {!usesSharedArtwork && (
            <img
              ...
              fetchPriority="high"
              onLoad={(event) => settleImage(event.currentTarget)}
              onError={() => {
                setImageStatus("error");
                onIntroReady?.();
              }}
          ...
          {imageStatus === "error" && (
            <span
              className="overview-image-fallback"
              role="img"
              aria-label={`Image unavailable for ${artwork.title}`}
            >
        ...
        <div className="overview-crops" aria-hidden="true">
          {[0, 1, 2, 3].map((index) => (
            <DetailCrop artwork={artwork} index={index} key={index} />
          ))}
        </div>
        <div className="overview-information">
          <h1 id="artwork-title" className="overview-final-title">
            {artwork.title}
          </h1>
