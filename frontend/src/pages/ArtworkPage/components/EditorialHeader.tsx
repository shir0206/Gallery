...
export function EditorialHeader({
  ...
  onPrevious,
  onNext,
  isArtworkNavigationDisabled = false,
  isGalleryWall = false,
}: EditorialHeaderProps) {
  let isSaved = false;
  try {
    isSaved = artwork
      ? window.localStorage.getItem(`shir-gallery:favorites:${artwork.id}`) ===
        "true"
      : false;
  } catch {
    /* optional storage */
  ...
  return (
    <header
      className={`editorial-header${
        isGalleryWall ? " editorial-header-gallery-wall" : ""
      }`}
    >
      <button
        type="button"
      ...
      <div className="editorial-header-actions">
        {onPrevious && onNext && (
          <nav
            className="editorial-artwork-navigation"
          ...
          >
            <button
              ...
              disabled={isArtworkNavigationDisabled}
            >
              ‹
            </button>
            <button
        ...
        <div className="editorial-tools" aria-label="Collection tools">
          ...
          <span title="Saved artworks">
            <HeartIcon />
          </span>
          <small>({isSaved ? 1 : 0})</small>
          <button
            type="button"
            title="Collection bag"
            aria-label={`Open collection bag, ${cartCount} ${
          ...
          <small className="editorial-bag-count">({cartCount})</small>
        </div>
      </div>
    </header>
  );
