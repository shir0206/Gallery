...
export function GalleryPage() {
  ...
  const [transitionPhase, setTransitionPhase] =
    useState<ArtworkTransitionPhase>("idle");
  const [transitionArtworkId, setTransitionArtworkId] = useState<string | null>(
    null
  );
  const [browseDirection, setBrowseDirection] =
    useState<BrowseDirection | null>(null);
  ...
  const routeState = location.state as {
    ...
    animateCartEntry?: boolean;
  } | null;
  const backgroundPath =
    routeState?.backgroundPath === "/collection" ? "/collection" : "/";
  const hasBackgroundRoute =
    routeState?.backgroundPath === "/" ||
    routeState?.backgroundPath === "/collection";
  const featureArtwork = featureArtworkSlug
    ? data?.artworks.find(
        (artwork) => artworkSlug(artwork.title) === featureArtworkSlug
      )
    : undefined;

  useEffect(() => {
    if (data && featureArtworkSlug && !featureArtwork)
  ...
  const openArtwork = (artworkId: string, sourceImage?: HTMLImageElement) => {
    ...
    const currentBackground =
      ...
      (location.pathname === "/search" && backgroundPath === "/collection")
        ? "/collection"
        : "/";
    ...
    if (sourceImage && currentBackground === "/") {
      ...
      const wallTrack = sourceImage.closest<HTMLElement>(
        ".artwork-viewer-track"
      );
      document.documentElement.style.setProperty(
        "--wall-scroll-offset",
        `${-(wallTrack?.scrollLeft ?? 0)}px`
      );
      const ratio =
        artwork.dimensions.width / Math.max(1, artwork.dimensions.height);
      const headerHeight = window.innerWidth <= 800 ? 64 : 72;
      const fallbackHeight = Math.min(
        (window.innerHeight - headerHeight) * 0.6,
        620
      );
      const fallbackWidth = fallbackHeight * ratio;
      const source =
        measured.width > 20 && measured.height > 20
          ? measured
          : {
              left: (window.innerWidth - fallbackWidth) / 2,
              top:
                headerHeight +
                (window.innerHeight - headerHeight - fallbackHeight) / 2,
              width: fallbackWidth,
      ...
      const isolateScale = Math.max(
        2.25,
        Math.min(3.4, (window.innerWidth * 0.86) / Math.max(1, source.width))
      );
      ...
      const isolate = cameraFor(
        isolateScale,
        galleryHeight - source.height * isolateScale * 0.25
      );
      ...
      requestAnimationFrame(() =>
        requestAnimationFrame(() => setGalleryCamera(isolate))
      );
      if (reducedMotion)
        transitionTimers.current.push(
          window.setTimeout(() => setTransitionPhase("ready"), 160)
        );
    } else {
    ...
    routerNavigate(`/artworks/${artworkSlug(artwork.title)}`, {
      state: { backgroundPath: currentBackground },
    });
  };
  const handleCameraSettled = useCallback(() => {
    ...
    const wordCount = Math.max(
      1,
      featureArtwork?.title.trim().split(/\s+/).length ?? 1
    );
  ...
  const closeArtworkToGallery = useCallback(
    (destination = backgroundPath) => {
      ...
      requestAnimationFrame(() =>
        requestAnimationFrame(() => setGalleryCamera({ x: 0, y: 0, scale: 1 }))
      );
    },
    [
      backgroundPath,
  ...
  const browseArtwork = (
    ...
    transitionTimers.current.push(
      window.setTimeout(() => setIsBrowseTransitioning(false), 525)
    );
  };
  useEffect(() => {
    if (!data || !featureArtwork) return;
    (["previous", "next"] as const).forEach((direction) => {
      const adjacentId = getAdjacentId(
        data.artworks,
        featureArtwork.id,
        direction
      );
  ...
  if (error) {
    return <GalleryStatus variant="error" message={error} onRetry={retry} />;
  }
  ...
  const cartArtworks = cartArtworkIds
    .map((artworkId) =>
      data.artworks.find((artwork) => artwork.id === artworkId)
    )
    .filter((artwork): artwork is (typeof data.artworks)[number] =>
      Boolean(artwork)
    );
  const addToCart = (artworkId: string) => {
    setCartArtworkIds((currentIds) =>
      currentIds.includes(artworkId) ? currentIds : [...currentIds, artworkId]
    );
    navigate("/cart");
  };
  const previewCartArtwork = (artworkId: string) => {
    ...
    setBrowseDirection(null);
    routerNavigate(`/artworks/${artworkSlug(artwork.title)}`, {
      state: { returnPath: "/cart", staticPreview: true },
    });
  };
  ...
  return (
    <>
      <EditorialHeader
        ...
        onGallery={() =>
          featureArtwork ? closeArtworkToGallery("/") : navigate("/")
        }
        ...
        isGalleryWall={
          location.pathname === "/" || transitionPhase === "closing"
        }
      />
      <Routes>
        <Route
          ...
          element={
            <Gallery
              data={data}
              onOpenFeature={openArtwork}
              onExitWall={() => navigate("/collection")}
              isCovered={Boolean(
                featureArtwork || isPurchaseOpen || isSearchOpen
              )}
              transitionArtworkId={transitionArtworkId}
              transitionPhase={transitionPhase}
              onCameraSettled={handleCameraSettled}
              suppressReveal={hasReturnedToGallery}
            />
          }
        />
        <Route
        ...
        <Route
          ...
          element={
            !hasBackgroundRoute ? (
              <div className="direct-artwork-background" aria-hidden="true" />
            ) : backgroundPath === "/collection" ? (
              <HomePage
                artworks={data.artworks}
                onSelectArtwork={openArtwork}
                onViewWall={() => navigate("/")}
              ...
              <Gallery
                data={data}
                focusedArtworkId={featureArtwork?.id}
                onOpenFeature={openArtwork}
                onExitWall={() => navigate("/collection")}
                isCovered
                transitionArtworkId={transitionArtworkId}
        ...
        <Route
          path="/about"
          element={<AboutPage onGallery={() => navigate("/")} />}
        />
        ...
        <Route
          ...
          element={
            <CartPage
              ...
              onContact={() => navigate("/contact")}
              onPreview={previewCartArtwork}
              onRemove={(artworkId) =>
                setCartArtworkIds((currentIds) =>
                  currentIds.filter((id) => id !== artworkId)
                )
              }
              onCheckout={() => setIsPurchaseOpen(true)}
              animateOnEntry={Boolean(routeState?.animateCartEntry)}
            />
          }
        />
      ...
      {featureArtwork && (
        <ArtworkPage
          ...
          transitionPhase={
            !hasBackgroundRoute && !routeState?.staticPreview
              ? "ready"
              : transitionPhase
          }
          ...
          onBack={() =>
            routeState?.returnPath === "/cart"
              ? closeArtworkToGallery("/cart")
              : navigate(backgroundPath)
          }
          onScrollBack={() =>
            closeArtworkToGallery(routeState?.returnPath || backgroundPath)
          }
          onPrevious={() =>
            browseArtwork(
              getAdjacentId(data.artworks, featureArtwork.id, "previous"),
      ...
      {isPurchaseOpen && cartArtworks.length > 0 && (
        <PurchaseFlow
          artworks={cartArtworks}
